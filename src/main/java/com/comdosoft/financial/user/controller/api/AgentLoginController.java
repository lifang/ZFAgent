...
public class AgentLoginController {
	...
	public Response sendPhoneVerificationCode(
		...
		try {
			...
			if (agentLoginService.judgeCityId(customer) > 0) {
				...
				if (customer.getId() == null) {
					...
					agent.setCardIdPhotoPath((String) map
							.get("cardIdPhotoPath"));
					if (agent.getTypes() == 1) {// 公司选项多出几个
						agent.setCompanyName((String) map.get("companyName"));
						agent.setBusinessLicense((String) map
								.get("businessLicense"));
						agent.setTaxRegisteredNo((String) map
								.get("taxRegisteredNo"));
						agent.setLicenseNoPicPath((String) map
								.get("licenseNoPicPath"));
	...
	public Response sizeUpImgCode(@RequestBody Map<String, String> map,
			HttpSession session) {
		try {
			if ((String) session.getAttribute("imageCode") == null) {
				return Response.getError("图片验证码错误！");
			} else {
				if (((String) session.getAttribute("imageCode"))
						.equalsIgnoreCase(map.get("imgnum"))) {
	...
	public Response modifyPassword(@RequestBody Customer customer) {
		...
		String oldPassword = agentLoginService.findCustomerPwdById(customer
				.getId());
